# -*- coding: utf-8 -*-
"""HeartBench 快速汇总：读取已有 _hb_*_out.json 生成最终结果（不重跑裁判）"""
import json
import os
import re
import sys
...
统一目录 = 本目录
缓存路径 = os.path.join(本目录, "heartbench_cache_2026.json")
结果路径 = os.path.join(本目录, "heartbench_results_2026.json")

# 从统一_run_heartbench.py 复制计算逻辑
...
def 汇总():
    缓存 = json.load(open(缓存路径, encoding="utf-8"))
    题目 = 缓存["题目"]
    全部汇总 = {}
    for 模式 in 模式列表:
        out = os.path.join(统一目录, f"_hb_{模式.replace(' ','')}_out.json")
        if not os.path.exists(out):
            print(f"⚠ 缺失 {模式} 裁判结果：{out}")
            全部汇总[模式] = {"overall_score": 0.0, "dimension_scores": {"人格": 0.0, "情绪": 0.0, "社交": 0.0, "道德": 0.0}, "有效题数": 0, "抽样数": len(题目)}
            continue
        裁判结果 = json.load(open(out, encoding="utf-8"))
        回复项 = 缓存["回复"][模式]
        四维 = {"人格": [], "情绪": [], "社交": [], "道德": []}
        overall_list = []
        得分记录 = []
        for i, cr in enumerate(裁判结果):
            if i >= len(回复项):
                break
            题 = next(t for t in 题目 if t["question_id"] == 回复项[i]["question_id"])
            detail = cr.get("detail")
            if detail is None:
                continue
            dim结果 = calculate_dimension_details(题["rubric"], detail, 特殊维度)
            if dim结果["has_special_hit"]:
                continue
            overall_list.append(dim结果["question_score"])
            得分记录.append({"question_id": 题["question_id"], "question_score": dim结果["question_score"], "detail": detail})
            for d in dim结果["dimension_details"]:
        ...
        print(f"{模式:<12} overall={全部汇总[模式]['overall_score']:.4f} 有效={len(overall_list)}")

    with open(结果路径, "w", encoding="utf-8") as f:
        json.dump({"模式汇总": 全部汇总, "方法": "HeartBench 官方 rubric 归一化", "种子": 2026}, f, ensure_ascii=False, indent=2)
    print(f"已保存 -> {结果路径}")
