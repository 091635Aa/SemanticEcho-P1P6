...
日志路径 = "i:\\Desktop\\\u8bed\u4e49\u56de\u54cd\\\u56fe\u7075\u6d4b\u8bd5\\\u7edf\u4e00\u57fa\u51c6\\HeartBench_log.txt"
结果路径 = "i:\\Desktop\\\u8bed\u4e49\u56de\u54cd\\\u56fe\u7075\u6d4b\u8bd5\\\u7edf\u4e00\u57fa\u51c6\\heartbench_results_2026.json"
样本数 = 40
...
def 生成缓存(题目, 模式列表, seed_base, runs, 缓存路径, λ=None):
    """阶段1：统一生成器（1.5B）生成全部模式回复 → 缓存 JSON"""
    from 统一生成器 import 生成器实例
    缓存 = {"题目": 题目, "seed_base": seed_base, "runs": runs, "回复": {}}
    for 模式 in 模式列表:
        记录日志(f"──── 生成 [{模式}] ────")
        回复列表 = []
        for run_idx in range(runs):
            ...
            for i, 题 in enumerate(题目):
                回应 = 生成回应(题["dialogue"], 模式=模式, 轮次=i, λ覆盖=λ, 种子=seed_offset)
                回复列表.append({"run": run_idx, "question_id": 题["question_id"], "回应": 回应})
...
def 裁判汇总(题目, 缓存路径, 模式列表, runs, λ=None):
    """阶段2：读缓存 → 子进程裁判 → 汇总"""
    import subprocess
    ...
    for 模式 in 模式列表:
        ...
        多次运行明细[模式] = []
        for run_idx in range(runs):
            seed_offset = 缓存.get("seed_base", 2026) + run_idx * 100
            生成结果 = []
            for 项 in 缓存["回复"][模式]:
                if 项["run"] != run_idx:
                    continue
                题 = next(t for t in 题目 if t["question_id"] == 项["question_id"])
                生成结果.append({"题": 题, "回应": 项["回应"]})
            请求列表 = []
            for i, g in enumerate(生成结果):
                题, 回应 = g["题"], g["回应"]
                对话文本 = "\n".join(f"{x['role']}: {x['content']}" for x in 题["dialogue"])
                请求列表.append({"对话": 对话文本, "回复": 回应, "rubric": 题["rubric"]})
            裁判子进程 = os.path.join(本目录, "裁判子进程.py")
            任务路径 = os.path.join(统一目录, f"_hb_{模式.replace(' ','')}.json")
            输出路径 = os.path.join(统一目录, f"_hb_{模式.replace(' ','')}_out.json")
            with open(任务路径, "w", encoding="utf-8") as f:
                json.dump({"裁判": "heartbench", "请求": 请求列表}, f, ensure_ascii=False)
            subprocess.run([sys.executable, 裁判子进程, 任务路径, 输出路径], check=True)
            with open(输出路径, encoding="utf-8") as f:
                裁判结果 = json.load(f)
            得分记录 = []
            早停淘汰 = False
            for i, (g, cr) in enumerate(zip(生成结果, 裁判结果)):
                题, 回应 = g["题"], g["回应"]
...
def main():
    ...
    ap.add_argument("--仅生成", type=str, default=None, help="只生成回复缓存到指定 JSON")
    ap.add_argument("--仅裁判", type=str, default=None, help="只裁判（读缓存 JSON）")
    args = ap.parse_args()
    模式列表 = 七模式列表 if args.模式 == "全部" else [args.模式]
    runs = max(1, args.runs)

    if os.path.exists(日志路径):
        os.remove(日志路径)
    记录日志(f"=== HeartBench 评测开始（模式：{模式列表}，λ={args.λ}，runs={runs}）===")
    ...
    缓存路径 = args.仅生成 or args.仅裁判
    if args.仅生成:
        生成缓存(题目, 模式列表, args.seed_base, runs, 缓存路径, λ=args.λ)
    elif args.仅裁判:
        裁判汇总(题目, 缓存路径, 模式列表, runs, λ=args.λ)
    else:
        print("请指定 --仅生成 缓存.json 或 --仅裁判 缓存.json（两阶段分开跑，避免 1.5B 与 7B 同进程）")
