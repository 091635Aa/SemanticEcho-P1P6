#!/usr/bin/env python3
"""
stress_test26.py — 第二十六轮：T=204800 极限

stress_test25 最优：
  cascade7+K+LPF T=102400 → avg=+0.6333 (+63.33%), p2p=+0.2105, Universal=YES
  T 翻倍继续推进 (+0.0453)

T-scaling 趋势：
  6400→0.4509, 12800→0.4516(+0.0007),
  25600→0.5277(+0.0761), 51200→0.5880(+0.0603),
  102400→0.6333(+0.0453)
  → 增量递减但仍在上升
...
def main():
    out = os.path.join(os.path.dirname(__file__), "stress_test26.json")
    print("=" * 120)
    print("第二十六轮：T=204800 极限 (cascade7+K+LPF)")
    print("=" * 120, flush=True)
    all_r = {}

    # ── T=204800 cascade7 ──
    run_round(0, "cascade7+K+LPF T=204800", lambda: BestCombo(n_cascade=7),
              T=204800, seeds=SEEDS_2, all_r=all_r, out_path=out)

    # ── T=204800 cascade5 (对比) ──
    run_round(1, "cascade5+K+LPF T=204800", lambda: BestCombo(n_cascade=5),
              T=204800, seeds=SEEDS_2, all_r=all_r, out_path=out)
