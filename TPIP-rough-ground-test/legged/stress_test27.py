#!/usr/bin/env python3
"""
stress_test27.py — 第二十七轮：T=409600 + 深级联变体

stress_test26 最优：
  cascade7+K+LPF T=204800 → avg=+0.6503 (+65.03%)
  T 翻倍增量递减 (+0.0170)

本轮：
  1. T=409600 单种子极限 (期望 ~+0.66~0.67)
  2. T=102400 cascade9/11/13 变体 (低成本，sweet spot)
"""
...
def run_round(round_num, name, make_fn, T=800, seeds=SEEDS_2, all_r=None, out_path=None):
    results = {}
...
def main():
    out = os.path.join(os.path.dirname(__file__), "stress_test27.json")
    print("=" * 120)
    print("第二十七轮：T=409600 极限 + 深级联变体")
    print("=" * 120, flush=True)
    all_r = {}

    # ── T=102400 深级联变体 (sweet spot) ──
    for n in [9, 11, 13, 15]:
        run_round(0, f"cascade{n}+K+LPF T=102400", lambda n=n: BestCombo(n_cascade=n),
                  T=102400, seeds=SEEDS_2, all_r=all_r, out_path=out)

    # ── T=409600 cascade7 (1 种子极限) ──
    run_round(1, "cascade7+K+LPF T=409600", lambda: BestCombo(n_cascade=7),
              T=409600, seeds=SEEDS_1, all_r=all_r, out_path=out)
