#!/usr/bin/env python3
"""
stress_test28.py — 第二十八轮：T=819200 单种子极限

stress_test27 结果：
  T=409600 (1 seed): avg=+0.6376, std=+0.5606, trans=+0.7146, p2p=+0.4433
  关键发现：长 T 让 p2p 大幅提升！
  但 std/trans 已饱和 (0.71~0.72)

本轮：T=819200 单种子极限，看 p2p 能否继续增长
"""
...
def main():
    out = os.path.join(os.path.dirname(__file__), "stress_test28.json")
    print("=" * 120)
    print("第二十八轮：T=819200 单种子极限 (cascade7+K+LPF)")
    print("=" * 120, flush=True)
    all_r = {}

    # ── T=819200 cascade7 ──
    run_round(0, "cascade7+K+LPF T=819200", lambda: BestCombo(n_cascade=7),
              T=819200, seeds=SEEDS_1, all_r=all_r, out_path=out)
