#!/usr/bin/env python3
"""
stress_test24.py — 第二十四轮：极致突破 + 增量保存

stress_test23 部分结果：
  R1 cascade7+Kalman+LPF T=25600 → avg=+0.5270 (+52.70%) ★ 新最优!
  R1 cascade5+Kalman+LPF T=25600 → avg=+0.5171 (+51.71%)
  cascade7 > cascade5 在 T=25600 下！
  (T=51200 测试因 timeout 被杀)

策略：
  1. 每测一个 round 立即保存 JSON
  2. 测试 cascade7/9/11 在 T=25600/51200
  3. 最少必要测试，避免 timeout
"""
...
def run_round(round_num, name, make_fn, T=800, seeds=SEEDS_5, all_r=None, out_path=None):
    results = {}
...
# 复用 AdaptiveLPF & KalmanSmoother (与 stress_test22/23 同实现)
...
def main():
    out = os.path.join(os.path.dirname(__file__), "stress_test24.json")
    print("=" * 120)
    print("第二十四轮：极致突破 (cascade7/9/11 @ T=25600/51200)")
    print("=" * 120, flush=True)
    all_r = {}

    # ── T=25600 cascade7 确认 (上轮最优) ──
    run_round(0, "cascade7+K+LPF T=25600", lambda: BestCombo(n_cascade=7),
              T=25600, seeds=SEEDS_3, all_r=all_r, out_path=out)

    # ── T=25600 cascade9 ──
    run_round(1, "cascade9+K+LPF T=25600", lambda: BestCombo(n_cascade=9),
              T=25600, seeds=SEEDS_3, all_r=all_r, out_path=out)

    # ── T=25600 cascade11 ──
    run_round(2, "cascade11+K+LPF T=25600", lambda: BestCombo(n_cascade=11),
              T=25600, seeds=SEEDS_3, all_r=all_r, out_path=out)

    # ── T=51200 cascade7 ──
    run_round(3, "cascade7+K+LPF T=51200", lambda: BestCombo(n_cascade=7),
              T=51200, seeds=SEEDS_3, all_r=all_r, out_path=out)

    # ── T=51200 cascade9 ──
    run_round(4, "cascade9+K+LPF T=51200", lambda: BestCombo(n_cascade=9),
              T=51200, seeds=SEEDS_3, all_r=all_r, out_path=out)
