#!/usr/bin/env python3
"""
stress_test25.py — 第二十五轮：T=102400 极限突破

stress_test24 最优：
  cascade7+K+LPF T=51200 → avg=+0.5880 (+58.80%), p2p=+0.2081, Universal=YES
  T 翻倍再次大跳跃 (+0.0603) → 趋势未饱和！

本轮目标：
  测试 T=102400 看趋势是否继续 → 期望 ~+0.65
"""
...
def run_round(round_num, name, make_fn, T=800, seeds=SEEDS_3, all_r=None, out_path=None):
    results = {}
...
def main():
    out = os.path.join(os.path.dirname(__file__), "stress_test25.json")
    print("=" * 120)
    print("第二十五轮：T=102400 极限突破 (cascade7+K+LPF)")
    print("=" * 120, flush=True)
    all_r = {}

    # ── T=102400 cascade7 (期望 +0.65) ──
    run_round(0, "cascade7+K+LPF T=102400", lambda: BestCombo(n_cascade=7),
              T=102400, seeds=SEEDS_3, all_r=all_r, out_path=out)

    # ── T=102400 cascade5 (对比) ──
    run_round(1, "cascade5+K+LPF T=102400", lambda: BestCombo(n_cascade=5),
              T=102400, seeds=SEEDS_3, all_r=all_r, out_path=out)
